...
def smoke_generators() -> None:
    print("generator smoke skipped: invoice skill has no generate_*.py entry points")
...
def smoke_catalog_renderers() -> None:
    ...
    if not scripts_dir.exists():
        ...
        return
    renderer = scripts_dir / "render_invoice.py"
    if not renderer.exists():
        raise RuntimeError("scripts/render_invoice.py is required")
    ...
    run([sys.executable, str(renderer), "--list"])
    if first_template:
        run([sys.executable, str(renderer), "--template", first_template, "--var", "smoke=value", "--no-pdf"])
